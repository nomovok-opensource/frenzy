...
<!--   
This transform generates Frenzy unit test code from a language independent
test representation.
...
saxon -o some_test.cpp someTest.xml test-to-frenzy.xsl
...
<xsl:stylesheet version="1.0" 
    ...
    <xsl:param name="package">w3domts</xsl:param>
...
BOOST_AUTO_TEST_SUITE(<xsl:value-of select="$package"/>)
...
/*
This source file was generated by test-to-frenzy.xsl
...
<!--   produces comments for metadata elements   -->
...
<xsl:text> * See also: </xsl:text><xsl:value-of select="@resource"/><xsl:text>
</xsl:text>
</xsl:for-each>
...
<xsl:text>BOOST_AUTO_TEST_CASE(</xsl:text><xsl:value-of select="@name"/>
    <xsl:text>)
{
</xsl:text>
...
<xsl:template match="*[local-name()='length' and @interface='DOMString']" mode="body">
    ...
    <xsl:text> = </xsl:text>
    <xsl:text>deref_if_optional(</xsl:text>
...
<xsl:template match="*[local-name()='var']" mode="body">
    ...
    <xsl:text>
      ...
      <xsl:text>(void) </xsl:text><xsl:value-of select="$varname"/>
    <xsl:text>;
    </xsl:text>
</xsl:template>
...
<xsl:template match="*[local-name()='append']" mode="body">
    ...
    <xsl:choose>
      <xsl:when test="$vardefs[@name=$varname]/@type = 'List'">
	<xsl:text>.push_back(</xsl:text>
      </xsl:when>
      <xsl:otherwise>
	<xsl:text>.insert(</xsl:text>
...
<xsl:template match="*[local-name()='assertInstanceOf']" mode="body">
    <xsl:param name="vardefs"/>
    <xsl:text>BOOST_CHECK(convert&lt;</xsl:text>
    <xsl:call-template name="produce-type">
        <xsl:with-param name="type" select="@type"/>
    </xsl:call-template>
    <xsl:text>&gt;(</xsl:text>
...
<xsl:template match="*[local-name()='assertEquals']" mode="body">
    ...
    <xsl:choose>
	  ...
	  <xsl:text>BOOST_CHECK(ascii_case_insensitive_equals(</xsl:text>
	  <xsl:text>deref_if_optional(</xsl:text>
	  <xsl:value-of select="@expected"/>
	  <xsl:text>), </xsl:text>
	  <xsl:text>deref_if_optional(</xsl:text>
	  ...
	  <xsl:choose>
	    <xsl:when test="@context = 'attribute'">
	      <xsl:text>BOOST_CHECK(ascii_case_insensitive_equals(</xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
	      <xsl:value-of select="@expected"/>
	      <xsl:text>), </xsl:text>
	    ...
	    <xsl:otherwise>
	      <xsl:text>BOOST_CHECK_EQUAL(ascii_toupper(</xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
	      <xsl:value-of select="@expected"/>
	      <xsl:text>)), </xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
...
<xsl:template match="*[local-name()='for-each']" mode="body">
    ...
    <xsl:choose>
    	...
    	<xsl:otherwise>
	  <xsl:call-template name="retval-cast">
	    <xsl:with-param name="variable" select="$indexvar"/>
	    <xsl:with-param name="vartype" select="$memberType"/>
	    <xsl:with-param name="rettype" select="$vartype"/>
	  </xsl:call-template>
	  <xsl:call-template name="produce-indexer">
	    <xsl:with-param name="indexvar" select="$indexvar"/>
	    <xsl:with-param name="vartype" select="$vartype"/>
	  </xsl:call-template>
	  <xsl:call-template name="retval-cast-end">
	    <xsl:with-param name="variable" select="$indexvar"/>
	    <xsl:with-param name="vartype" select="$memberType"/>
	    <xsl:with-param name="rettype" select="$vartype"/>
	  </xsl:call-template>
...
<xsl:template name="produce-type">
    <xsl:param name="type"/>
    <xsl:choose>
        <xsl:when test="$type='DOMString'">boost::optional&lt;ustring&gt;</xsl:when>
        ...
        <xsl:when test="$type='Collection'">std::set&lt;ustring&gt;</xsl:when>
        <xsl:when test="$type='List'">std::vector&lt;ustring&gt;</xsl:when>
	...
	<xsl:when test="$type='int'">int</xsl:when>
        <xsl:otherwise>
            <xsl:value-of select="$type"/><xsl:text>p</xsl:text>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<xsl:template match="*[local-name()='load' and not(@interface)]" mode="body">
    ...
    <xsl:value-of select="@var"/>
    <xsl:text> = </xsl:text>
    <xsl:text>load("test/w3domts/</xsl:text>
...
<xsl:template name="retval-cast-end">
    <xsl:param name="vartype"/>
    <xsl:param name="rettype"/>
    <xsl:param name="variable"/>
    <xsl:choose>
	...
	<xsl:choose>
	   <xsl:when test="$startsWithIs">ls ls /.
        	<xsl:call-template name="build-accessor">
            		<xsl:with-param name="prefix">->set_</xsl:with-param>
...
<xsl:template match="*[local-name()='equals']" mode="condition">
    <xsl:choose>
	  ...
	  <xsl:choose>
	    <xsl:when test="@context = 'attribute'">
	      <xsl:text>ascii_case_insensitive_equals(</xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
	      <xsl:value-of select="@expected"/>
	      <xsl:text>), </xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
	    ...
	    <xsl:otherwise>
	      <xsl:text>ascii_toupper(</xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
	      <xsl:value-of select="@expected"/>
	      <xsl:text>)) == </xsl:text>
	      <xsl:text>deref_if_optional(</xsl:text>
...
<xsl:template match="*[local-name()='isNull']" mode="condition">
    <xsl:text>(!</xsl:text><xsl:value-of select="@obj"/><xsl:text>)</xsl:text>
</xsl:template>
...
<xsl:template match="*[local-name()='contentType']" mode="condition">
  <!-- hard-coded value of text/html here -->
    <xsl:text>(std::string("</xsl:text><xsl:value-of select="@type"/><xsl:text>") == "text/html")</xsl:text>
</xsl:template>
